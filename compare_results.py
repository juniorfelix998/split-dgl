...
def create_comparison_plots(dgl_results, baseline_results):
    ...
    if dgl_results:
        ...
        ax1.plot(rounds_dgl, dgl_results['train_loss'], 'b-o', 
                label='DGL Split Learning', linewidth=2, markersize=4)
    if baseline_results:
        rounds_baseline = list(range(1, len(baseline_results['train_loss']) + 1))
        ax1.plot(rounds_baseline, baseline_results['train_loss'], 'r-s', 
                label='Standard Split Learning', linewidth=2, markersize=4)
    ...
    if dgl_results:
        ax2.plot(rounds_dgl, dgl_results['test_loss'], 'b-o', 
                label='DGL Split Learning', linewidth=2, markersize=4)
    if baseline_results:
        ax2.plot(rounds_baseline, baseline_results['test_loss'], 'r-s', 
                label='Standard Split Learning', linewidth=2, markersize=4)
    ...
    if dgl_results:
        ax3.plot(rounds_dgl, dgl_results['train_acc'], 'b-o', 
                label='DGL Split Learning', linewidth=2, markersize=4)
    if baseline_results:
        ax3.plot(rounds_baseline, baseline_results['train_acc'], 'r-s', 
                label='Standard Split Learning', linewidth=2, markersize=4)
    ...
    if dgl_results:
        ax4.plot(rounds_dgl, dgl_results['test_acc'], 'b-o', 
                label='DGL Split Learning', linewidth=2.5, markersize=5)
    if baseline_results:
        ax4.plot(rounds_baseline, baseline_results['test_acc'], 'r-s', 
                label='Standard Split Learning', linewidth=2.5, markersize=5)
    
    ax4.set_xlabel('Round', fontsize=12, fontweight='bold')
    ax4.set_ylabel('Test Accuracy (%)', fontsize=12, fontweight='bold')
    ax4.set_title('Test Accuracy Comparison (Key Metric)', fontsize=14, fontweight='bold')
    ...
    bars1 = ax5.bar(x - width/2, train_accs, width, label='Train Accuracy', 
                    color='steelblue', alpha=0.8)
    bars2 = ax5.bar(x + width/2, test_accs, width, label='Test Accuracy', 
...
def create_convergence_analysis(dgl_results, baseline_results):
    ...
    for target in target_accuracies:
        if dgl_results:
            ...
            if len(dgl_rounds) > 0:
                ax.scatter(target, dgl_rounds[0] + 1, marker='o', s=150, 
                          c='blue', label=f'DGL' if target == target_accuracies[0] else "")
        
        if baseline_results:
            baseline_rounds = np.where(np.array(baseline_results['test_acc']) >= target)[0]
            if len(baseline_rounds) > 0:
                ax.scatter(target, baseline_rounds[0] + 1, marker='s', s=150, 
                          c='red', label=f'Standard' if target == target_accuracies[0] else "")
    ...
    ax.set_title('Convergence Speed: Rounds to Target Accuracy', 
    ...
    if dgl_results and len(dgl_results['test_acc']) > 1:
        dgl_improvement = [dgl_results['test_acc'][i] - dgl_results['test_acc'][i-1] 
                          for i in range(1, len(dgl_results['test_acc']))]
        ax.plot(range(2, len(dgl_results['test_acc']) + 1), dgl_improvement, 
               'b-o', label='DGL Split Learning', alpha=0.7, linewidth=2)
    
    if baseline_results and len(baseline_results['test_acc']) > 1:
        baseline_improvement = [baseline_results['test_acc'][i] - baseline_results['test_acc'][i-1] 
                               for i in range(1, len(baseline_results['test_acc']))]
        ax.plot(range(2, len(baseline_results['test_acc']) + 1), baseline_improvement, 
               'r-s', label='Standard Split Learning', alpha=0.7, linewidth=2)
...
def print_detailed_comparison(dgl_results, baseline_results):
    ...
    if dgl_results and baseline_results:
        ...
        if abs(acc_diff) < 1.0:
            findings.append(f"✓ Both methods achieve similar accuracy (Δ = {acc_diff:+.2f}%)")
        elif acc_diff > 0:
            findings.append(f"✓ DGL achieves {acc_diff:.2f}% HIGHER test accuracy!")
        else:
            findings.append(f"✗ Standard SL achieves {-acc_diff:.2f}% higher test accuracy")
        
        # Communication efficiency
        findings.append(f"✓ DGL eliminates gradient communication from server to client")
        findings.append(f"  → Reduces communication overhead by ~50% (no backward gradients)")
        ...
        findings.append(f"  → Additional parameters: ~4-5% of main network (mlp-sr type)")
...
def create_architecture_comparison():
    ...
    ax = axes[0]
    ax.text(0.5, 0.9, 'STANDARD SPLIT LEARNING', ha='center', fontsize=16, 
            fontweight='bold', transform=ax.transAxes)
    
    # Client
    client_box = plt.Rectangle((0.3, 0.6), 0.4, 0.2, fill=True, 
                               facecolor='lightblue', edgecolor='black', linewidth=2)
    ax.add_patch(client_box)
    ax.text(0.5, 0.7, 'Client Model\n(CNN Layers)', ha='center', va='center', 
           fontsize=11, fontweight='bold', transform=ax.transAxes)
    
    # Server
    server_box = plt.Rectangle((0.3, 0.2), 0.4, 0.2, fill=True, 
                               facecolor='lightcoral', edgecolor='black', linewidth=2)
    ax.add_patch(server_box)
    ax.text(0.5, 0.3, 'Server Model\n(CNN + Classifier)', ha='center', va='center', 
           fontsize=11, fontweight='bold', transform=ax.transAxes)
    ...
    ax.text(0.55, 0.5, 'Smashed Data\n(Forward)', ha='left', va='center',
           fontsize=10, color='green', transform=ax.transAxes)
    
    # Backward arrow
    ax.annotate('', xy=(0.48, 0.62), xytext=(0.48, 0.38),
               arrowprops=dict(arrowstyle='->', lw=3, color='red'),
               transform=ax.transAxes)
    ax.text(0.28, 0.5, 'Gradients\n(Backward)', ha='right', va='center',
    ...
    # DGL Split Learning
    ax = axes[1]
    ax.text(0.5, 0.9, 'DGL SPLIT LEARNING (BP-FREE)', ha='center', fontsize=16, 
            fontweight='bold', transform=ax.transAxes)
    
    # Client with auxiliary
    client_box = plt.Rectangle((0.25, 0.55), 0.35, 0.2, fill=True, 
                               facecolor='lightblue', edgecolor='black', linewidth=2)
    ax.add_patch(client_box)
    ax.text(0.425, 0.65, 'Client Model\n(CNN Layers)', ha='center', va='center', 
           fontsize=11, fontweight='bold', transform=ax.transAxes)
    
    # Auxiliary network
    aux_box = plt.Rectangle((0.65, 0.55), 0.25, 0.2, fill=True, 
                            facecolor='lightyellow', edgecolor='black', linewidth=2)
    ax.add_patch(aux_box)
    ax.text(0.775, 0.65, 'Auxiliary\nNetwork', ha='center', va='center', 
           fontsize=10, fontweight='bold', transform=ax.transAxes)
    
    # Connection to auxiliary
    ax.annotate('', xy=(0.65, 0.65), xytext=(0.6, 0.65),
               arrowprops=dict(arrowstyle='->', lw=2, color='purple'),
               transform=ax.transAxes)
    ax.text(0.625, 0.7, 'Local\nGradients', ha='center', va='bottom',
           fontsize=9, color='purple', transform=ax.transAxes)
    
    # Server
    server_box = plt.Rectangle((0.3, 0.15), 0.4, 0.2, fill=True, 
                               facecolor='lightcoral', edgecolor='black', linewidth=2)
    ax.add_patch(server_box)
    ax.text(0.5, 0.25, 'Server Model\n(CNN + Classifier)', ha='center', va='center', 
           fontsize=11, fontweight='bold', transform=ax.transAxes)
    
    # Forward arrow only
    ax.annotate('', xy=(0.5, 0.55), xytext=(0.5, 0.35),
               arrowprops=dict(arrowstyle='->', lw=3, color='green'),
               transform=ax.transAxes)
    ax.text(0.55, 0.45, 'Smashed Data\n(Forward ONLY)', ha='left', va='center',
           fontsize=10, color='green', fontweight='bold', transform=ax.transAxes)
    
    # No backward arrow - that's the point!
    ax.text(0.15, 0.45, 'NO GRADIENTS\nFROM SERVER!', ha='center', va='center',
