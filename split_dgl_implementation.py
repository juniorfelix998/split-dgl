...
class AuxiliaryClassifier(nn.Module):
    ...
    def __init__(self, input_channels, spatial_size, num_classes=10, aux_type="mlp-sr"):
...
def main():
    ...
    global_client_model = ClientSideModel(
        num_classes=config.num_classes, aux_type=config.aux_type
    ).to(device)

    # Global server model
    global_server_model = ServerSideModel(num_classes=config.num_classes).to(device)

    print(f"\nClient Model:")
    print(
        f"  - Parameters: {sum(p.numel() for p in global_client_model.parameters()):,}"
    )
    print(f"  - Auxiliary Network: {config.aux_type}")
    print(f"\nServer Model:")
    print(
        f"  - Parameters: {sum(p.numel() for p in global_server_model.parameters()):,}"
    )
