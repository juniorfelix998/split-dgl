...
def train_server_baseline(server_model, smashed_data, labels, criterion, 
                         optimizer, idx, local_epoch, local_epoch_count, len_batch):
    """
...
def evaluate_server_baseline(server_model, smashed_data, labels, criterion,
    ...
    def train_standard(self, server_model, server_optimizer, global_round):
        ...
        for local_epoch in range(self.config.local_epochs):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                smashed_data_with_grad = smashed_data.clone().detach().requires_grad_(True)
...
def main():
    ...
    global_server_model = ServerSideModel_Baseline(num_classes=config.num_classes).to(device)
    
    print(f"\nClient Model Parameters: {sum(p.numel() for p in global_client_model.parameters()):,}")
    print(f"Server Model Parameters: {sum(p.numel() for p in global_server_model.parameters()):,}")
